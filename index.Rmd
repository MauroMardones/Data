...
bit <- read_excel("Artesanal_Historica/datos Bacalao historico 1997-2022CTP/BITACORAS ESPINEL bacalao.xlsx", sheet = "BITACORAS_ESPINEL_bacalao",  guess_max = 100000)
...
landing <- read_excel("Artesanal_Historica/DESEMBARQUE HISTÓRICO.xlsx", 
    skip = 2)
bit8696<- read.csv2("bit_art_86_20.csv",sep=",", header = T)


bit$POTENCIA_MOTOR_EQ <- as.numeric(bit$POTENCIA_MOTOR_EQ)
```
...
cpuefipa <- ggplot(bit8696 %>%
                     filter(ANO<1997) %>% 
                     mutate(CPUE=as.double(CPUE)) %>%
                  group_by(ANO, AREA) %>% 
...
```{r fig.cap= "Cambios espaciotempiorales del esfuerzo pesquero en bacalao APA. Linea roja representa el 47ºS"}
## Plot final
mas1 <- ggplot() +
...
```{r fig.cap= "Cambios espaciotempiorales del rendimiento pesquero en bacalao APA. Linea roja representa el 47ºS"}
## Plot final
mas2 <- ggplot() +
  geom_sf(data=joindat %>% 
...
En función de los datos analizados, el modelo propuesto es un enfoque de producción global estao espacio descrito por @Pedersen2017 y @ICES2015.
